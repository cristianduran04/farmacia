<div class="container is-fluid mb-6">
	...
	<h2 class="subtitle"><i class="fas fa-cart-plus fa-fw"></i> &nbsp; Nueva venta a Credito</h2>
</div>
    ...
    <div class="columns">
        ...
        <div class="column is-one-quarter">
            ...
            <?php } ?>
                ...
                <?php } ?>
                <div class="control mb-5">
                    <label>Tipo de pago <?php echo CAMPO_OBLIGATORIO; ?></label><br>
                    <div class="select">
                        <select name="venta_tipo">
                ...
                <div class="control mb-5">
    <label>Abonado por el cliente <?php echo CAMPO_OBLIGATORIO; ?></label>
    <input class="input" type="text" pattern="[0-9.]{1,10}" maxlength="10" name="venta_abono" id="venta_abono" value="0">
</div>

<div class="control mb-5">
    <label>Total a deber <?php echo CAMPO_OBLIGATORIO; ?></label>
    <input class="input" type="text" id="venta_total_deber" value="<?php echo $_SESSION['venta_total']; ?>" readonly>
</div>

                <div class="control mb-5">
                    <label>Descuento <?php echo CAMPO_OBLIGATORIO; ?></label>
                    <input class="input" type="text" pattern="[0-9.]{1,5}" maxlength="5" name="venta_descuento" value="0">
                </div>

                <div class="control mb-5">
                    <label>Total <?php echo CAMPO_OBLIGATORIO; ?></label>
                    <input class="input" type="text" value="<?php echo MONEDA_SIMBOLO . number_format($_SESSION['venta_total'], MONEDA_DECIMALES, MONEDA_SEPARADOR_DECIMAL, MONEDA_SEPARADOR_MILLAR) . " " . MONEDA_NOMBRE; ?>" readonly>
                </div>
...
<script>
    ...
    function actualizar_cantidad(input_id, producto_codigo) {
        var cantidad = document.querySelector(input_id).value;
        // Aquí deberías agregar el código para actualizar la cantidad en la base de datos o en la sesión.
    }

    /* Detectar cuando se envía el formulario para agregar producto */
    let sale_form_barcode = document.querySelector("#sale-barcode-form");
    sale_form_barcode.addEventListener('submit', function(event){
        event.preventDefault();
        setTimeout('agregar_producto()',100);
    ...
    /* Detectar cuando escanea un código en formulario para agregar producto */
    let sale_input_barcode = document.querySelector("#sale-barcode-input");
    sale_input_barcode.addEventListener('paste',function(){
        setTimeout('agregar_producto()',100);
    });

    // Resto de las funciones JavaScript que proporcionaste

    /*----------  Calcular cambio  ----------*/

    ChatGPT
Para garantizar que el cambio se actualice correctamente al ingresar el abono por parte del cliente, necesitas ajustar la lógica en tu función JavaScript. Es posible que el evento keyup no se dispare como esperas. Vamos a modificar el código para que el cambio se calcule correctamente cuando se modifique el campo de abono.
...
function calcularCambio() {
    var abono = parseFloat(document.querySelector('input[name="venta_pagado"]').value);
    var total = parseFloat(document.querySelector('#venta_total_hidden').value.replace(/[^0-9.-]+/g,"")); // Extraemos solo los números del total

    // Verificar si el abono es un número válido
    if (isNaN(abono)) {
        abono = 0;
    }

    // Calcular el cambio
    var cambio = abono - total;

    // Actualizar el campo de "Total a deber"
    document.querySelector('input[name="venta_cambio"]').value = cambio.toFixed(<?php echo MONEDA_DECIMALES; ?>);
}

// Detectar cambios en el campo de abono
var abonoInput = document.querySelector('input[name="venta_pagado"]');
abonoInput.addEventListener('input', calcularCambio);



// Enlazar evento input al campo de abono
let venta_abono_input = document.querySelector("#venta_abono");
venta_abono_input.addEventListener('input', calcular_cambio)
